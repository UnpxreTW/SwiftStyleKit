...
// 原因：本檔是議題 1 / PR1 的一次性 codegen dev 工具、住在獨立 package（Tools/FormatRuleCodegen、
...
// MARK: - 一次性 codegen for FormatRule 型別安全地基（議題 1 / PR1）
//
...
//   2) 生成型別安全 enable/disable/poison overloads（FormatRule+SafeOverloads.swift）。
//
// 4 個特例（dry-run 挖出、沒做會編不過）：
...
//      生單一 passthrough factory、無 enable/disable/poison。
...
// disable overload 會缺引數編譯 fail——此即 PR1 必須處理的 edge。
...
func renderOverloads(_ caseInfo: CaseInfo) -> String {
	...
	case .deprecated:
		...
		"""
	case .globalOption:
		// 特例 4：無 Flag——維持單一 passthrough factory（mode 預設保留），無 enable/disable/poison
	...
	case .flagPlusParams:
		...
		\t}
		"""
		// 3. disable + 參數毒餌（@available unavailable、吐客製訊息）
		let poisonParams = caseInfo.extraParams.map { param -> String in
			let label = param.label.isEmpty ? "_ value" : param.label
			return "\(label): \(param.typeText)"
		}.joined(separator: ", ")
		let poisonOverload = """
		\t@available(*, unavailable, message: "rule 為 .disable 時不可帶 option（option 只在 .enable 有效）")
		\tpublic static func \(caseInfo.name)(rule: DisableToken, \(poisonParams)) -> FormatRule {
		\t\tfatalError("unavailable")
		\t}
		"""
		return [enableOverload, disableOverload, poisonOverload].joined(separator: "\n\n")
	}
}
...
//  型別安全 enable/disable/poison overloads。改規則簽名請改 FormatRule.swift 後重跑 codegen。
