...
private struct HoistPatternLetTests {

	@Test
	private func `hoistPatternLet .disable 返空陣列`() {
		let args = FormatRule.hoistPatternLet(rule: .disable, mode: .inline).cliArguments
		#expect(args.isEmpty)
	}

	@Test
	private func `hoistPatternLet .enable（mode 預設 .hoist）展開 --enable + --patternLet hoist`() {
	...
	@Test
	private func `hoistPatternLet .enable mode .inline 展開 --patternLet inline`() {
