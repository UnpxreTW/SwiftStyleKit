...
struct FormatRuleTests {
    ...
    func blankLineAfterSwitchCaseDisableWithMode() {
        let args = FormatRule.blankLineAfterSwitchCase(rule: .disable, .always).cliArguments
        #expect(args.isEmpty)
    }

    @Test("blankLineAfterSwitchCase .enable .multilineOnly 展開 --enable + mode multiline-only")
    func blankLineAfterSwitchCaseEnableMultilineOnly() {
        let args = FormatRule.blankLineAfterSwitchCase(rule: .enable, .multilineOnly).cliArguments
        #expect(args == [
    ...
    @Test("blankLineAfterSwitchCase .enable .always 展開 --enable + mode always")
    func blankLineAfterSwitchCaseEnableAlways() {
        let args = FormatRule.blankLineAfterSwitchCase(rule: .enable, .always).cliArguments
        #expect(args == [
            "--enable", "blankLineAfterSwitchCase",
