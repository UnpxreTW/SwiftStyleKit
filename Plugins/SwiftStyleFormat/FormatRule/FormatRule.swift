...
/// 格式規則
///
/// 對外是值型別 façade：消費者用 static func 工廠（``acronyms(rule:_:)`` 等）建立、
/// ``cliArguments`` 以 Mirror reflection 展開。實際資料存在 internal 巢狀 ``Storage`` enum、
/// 不入對外 interface。工廠正逐規則從過渡相容的 `rule: Flag` 遷往型別安全的 `.on` / `.off`。
public struct FormatRule {

	// MARK: Lifecycle

	/// 以後端表示建立。無 label：codegen 產 `.init(.acronyms(…))`。
	...
	/// 全部規則的後端聯集，每個 case ＝一條規則及其 ``Flag`` 與 option。
	///
	/// internal、不外露；對外名與型別安全由 ``FormatRule`` 的 static func 工廠提供。
	enum Storage {
	...
	/// 規則的後端表示。internal——公開 API 是工廠 func（見 FormatRule+SafeOverloads.swift），
	/// 此值不進對外 interface。
	let storage: Storage
