...
extension FormatRule {
	...
	/// 停用（不可帶 option）
	public static func docComments(_ state: OffToken) -> FormatRule {
		._storage(.docComments(rule: .disable))
	}
	...
	/// 停用（不可帶 option）
	public static func duplicateImports(_ state: OffToken) -> FormatRule {
		._storage(.duplicateImports(rule: .disable))
	}
	...
	/// 停用（不可帶 option）
	public static func elseOnSameLine(_ state: OffToken) -> FormatRule {
		._storage(.elseOnSameLine(rule: .disable))
	}

	/// `.off` 誤帶 option 的編譯期診斷（命中即報錯、不會被呼叫）
	@available(*, unavailable, message: ".off 不可帶 option（option 只在 .on 有效）")
	public static func elseOnSameLine(_ state: OffToken, elsePosition: ElsePosition = .sameLine, guardElse: GuardElsePosition = .nextLine) -> FormatRule {
		fatalError("unavailable")
	}
	...
	/// 停用（不可帶 option）
	public static func emptyBraces(_ state: OffToken) -> FormatRule {
		._storage(.emptyBraces(rule: .disable))
	}
	...
	/// 停用（不可帶 option）
	public static func enumNamespaces(_ state: OffToken) -> FormatRule {
		._storage(.enumNamespaces(rule: .disable))
	}
	...
	/// 停用（不可帶 option）
	public static func extensionAccessControl(_ state: OffToken) -> FormatRule {
		._storage(.extensionAccessControl(rule: .disable))
	}
